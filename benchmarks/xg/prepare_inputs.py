...
def prepare_ce_psd(output: Path = DEFAULT_OUTPUT) -> Path:
    ...
    with urllib.request.urlopen(CE_PSD_URL, timeout=60) as response:
        content = response.read()
    digest = _sha256(content)
    if digest != CE_PSD_SHA256:
        raise RuntimeError(
            f"CE PSD digest mismatch: expected {CE_PSD_SHA256}, received {digest}"
        )
